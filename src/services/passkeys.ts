...
async function createPasskey(): Promise<PasskeyCredentialWithPubkeyCoordinates> {
  ...
  const passkeyWithCoordinates: PasskeyCredentialWithPubkeyCoordinates =
    Object.assign(passkeyCredential, {
      pubkeyCoordinates: {
        x: `0x${Buffer.from(exportedKeyWithXYCoordinates.x, 'base64').toString(
          'hex'
        )}`,
        y: `0x${Buffer.from(exportedKeyWithXYCoordinates.y, 'base64').toString(
          'hex'
        )}`
      }
    })
...
export { createPasskey, isLocalStoragePasskey, toLocalStorageFormat }
