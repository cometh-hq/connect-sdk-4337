...
import { extractPublicKeyFromAttestationObject } from "./webauthn/coseParser";
...
function getWebAuthnOrigin(tauriOptions: {
    ...
    throw new Error("No platform detected, cannot get WebAuthn origin");
}
 ...
 */
export function getTauriCreateFn(tauriOptions: {
    androidApkOrigin: string;
    rpId: string;
}): OxCreateFn {
    if (!isAndroid()) {
    ...
    return async (options) => {
        ...
        const creationOptions = toTauriCreationOptions(options);
        if (!creationOptions) return null;

        const origin = getWebAuthnOrigin(tauriOptions);
 ...
 * Returns undefined if not running in Tauri (ox will use default browser API)
 */
export function getTauriGetFn(tauriOptions: {
    androidApkOrigin: string;
    rpId: string;
}): OxGetFn {
    if (!isAndroid()) {
    ...
    return async (options) => {
        if (!options) return null;
        const requestOptions = toTauriRequestOptions(options);
